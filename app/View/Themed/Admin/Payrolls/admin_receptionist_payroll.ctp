...
<div class="row-fluid">
			  <div class="span6" style="padding-top:10px;">
				   ...
				   echo $this->form->create("Payroll",array('url' => array('controller' => 'payrolls', 'action' => 'receptionist_payroll' , $this->params['pass'][0]), 'type' => 'get'));
	...
	<thead>
		<tr>
			<th  width="5%">
			 ...
			 <th>
				  <strong><?php echo   "Weekly Payment"; ?></strong>
			 </th>
	...
	<tbody>
		...
		<?php
			...
			if (!empty($newDataArray)) {
				...
				foreach ($newDataArray as $k => $order) {
					...
					$WeekPaymentToal = $WeekPaymentToal + $order['payment'];
		?>
		<tr>
			<td>
				<?php echo $j; ?>
			...
			<td> <?php  if(isset($order['payment']) && $order['payment'] != 0){
		...
		<?php 	$j++;
			...
			<tr>
				...
				<td><strong><?php echo $WeekPaymentToal; ?></strong></td>
