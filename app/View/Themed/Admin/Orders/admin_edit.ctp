...
$(document).ready(function() {
    ...
    $(add_button_tag ).click(function(e){ //on add input button click
        ...
        if(x < max_fields_tag ){ //max input box allowed
			...
			content += '<div class="col-md-4 input-left">';
			...
			content += '<div class="col-md-4 input-right"><div class="input text required"><input type="input" name="data[Product][quantity][]" placeholder="Quantity" class="form-control marBottom10" style=""></div></div>';
			content += '<div class="col-md-2"><button type="submit" class="remove_field_tag btn custom-btn-index ">Remove</button></div>';
			content += '</div>';
...
<div class="row">
	...
	<?php
				   ...
				   if($this->data['Customer']['role_id'] == 5)
				   {
					   $cli = $this->data['Order']['customer_id'];
				   }else{
					    $cli = '';
				   }
				   if($this->data['Customer']['role_id'] == 3)
				   {
					   $emp = $this->data['Order']['customer_id'];
				   }else{
					    $emp = '';
				   }
					...
					foreach ($OrderProduct as $k => $productorder) {
						
					echo '<div class="clearDiv productDiv"></div><div style="margin:5px 0 5px 0" class="productDiv">';
					...
					?>
						...
						<a class="btn btn-default btn-select">
										<input type="hidden" class="btn-select-input" id="" name="data[Product][product_id][]" value="<?php echo $productorder['OrderProduct']['product_id']?>" />
						...
						?>
											...
											<li data-product-id="<?php echo $k; ?>" class="<?php if($k==$productorder['OrderProduct']['product_id']){ echo 'selected'; } ?>"><?php echo $product; ?></li>
						...
						<div class="col-md-4">
							<div class="input text required">
								<input type="input" name="data[Product][quantity][]" placeholder="Quantity" class="form-control marBottom10" style="" value="<?php echo $productorder['OrderProduct']['quantity']?>">
							</div>
							
						</div>
						<?php if(!empty($OrderProduct)){ ?>
						<div class="col-md-2"><button type="submit" class="remove_field_tag btn custom-btn-index ">Remove</button></div>
						<?php } ?>
					<?php
					echo '</div>';
					}
...
$('document').ready(function(){
	$("#OrderCId").change(function(){
		...
		$("#OrderEId option:selected").prop("selected", false)
	});
	$("#OrderEId").change(function(){
		...
		$("#OrderCId option:selected").prop("selected", false)
	});
});
</script>
