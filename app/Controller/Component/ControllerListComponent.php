...
class ControllerListComponent extends Component {
	 ...
	 * @return array
	 */
	public function get($deny=array()) {
		...
		if(!empty($deny)){
		foreach($deny as $k=>$v){
			if(isset($controllers[$k]) && !empty($controllers[$k])){
				foreach($controllers[$k] as $c=>$action){
					foreach($v as $ac){
						if($ac==$action)
							unset($controllers[$k][$c]);
					}
				}
			}
	...
	public function getConMet($deny=array(),$ncon) {
		$superParentActions = get_class_methods('Controller');
		$parentActions = get_class_methods('AppController');
		$controllers= array();
		$plugins = App::objects('plugins');
			...
			foreach ($pluginControllerClasses as $controller) {
				...
				if (!empty($actions)) {
                   ...
                   // pr($pluginAppContMethods);
                     foreach ($ncon as $controller) {
                                $controllername=str_replace('Controller', '',$controller);
	 ...
	 * @return array
	 */
	private function __getControllerMethods($controllername, $superParentActions, $parentActions, $p=null,$deny=array()) {
		if (empty($p)) {
	...
	public function _unsetController($array=array(),$controllers=array()){
	...
	public function getControllerWithMethods() {
        ...
        public function matchAvail($chk=array(),$dbcontroller=array()){
             //pr($chk);
             $flag = 0;
             if(!empty($dbcontroller)){
                  foreach($dbcontroller as $db){
					if(($chk['controller'] == $db['controller']) && ($chk['action'] == $db['action'])){
							$flag = 1;
							break;
					   }
